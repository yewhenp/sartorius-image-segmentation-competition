...
from src.constants import ConfigKeys as ck, HEIGHT, WIDTH
# START PROJECT IMPORTS
from src.data_processing.utilities import load_train_labels
...
def group_to_mask_image(group):
    ...
    for idx, line in group.iterrows():
        ...
        for i in range(len(erosion_kern)):
            curr_imgs.append(
                cv2.erode(curr_imgs[i - 1], np.ones((erosion_kern[i], erosion_kern[i]), np.uint8), iterations=1))
...
def main(cnf: Dict):
    ...
    for idx, group in tqdm(groups):
        ...
        with open(f"masks_water/mask_{idx}.pkl", "wb") as f:
            pickle.dump(mask, f)
